...
/**
 * Aplica valores por defecto para la configuración del superadmin.
 * 
 * Si no existe configuración de superadmin, no se aplica nada.
 * Si existe pero falta el campo 'enabled', se establece en true por defecto.
 */
export function applySuperadminDefaults(cfg: OpenClawConfig): OpenClawConfig {
  const superadmin = cfg.gateway?.superadmin;
  
  // Si no hay configuración de superadmin, no hacer nada
  if (!superadmin) {
    return cfg;
  }
  
  // Si ya tiene el campo enabled definido, no modificar
  if (typeof superadmin.enabled === 'boolean') {
    return cfg;
  }
  
  // Establecer enabled: true por defecto
  return {
    ...cfg,
    gateway: {
      ...cfg.gateway,
