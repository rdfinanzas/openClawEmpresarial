...
export async function processMessage(params: {
  ...
  // Build system prompt from WhatsApp config (enterprise mode)
  const whatsappConfig = params.cfg.channels?.whatsapp;
  const systemPromptParts = [
    // Global WhatsApp system prompt
    whatsappConfig?.systemPrompt?.trim() || null,
    // Per-group system prompt if in a group
    params.msg.chatType === "group" && params.msg.groupSubject
      ? whatsappConfig?.groups?.[params.msg.groupSubject]?.systemPrompt?.trim() || null
      : null,
  ].filter((entry): entry is string => Boolean(entry));
