...
import { getChannelRole } from "../channels/roles.js";
...
export function createOpenClawCodingTools(options?: {
  ...
  // Apply channel role-based tool filtering for public access restriction
  // This filter is applied after all other policies to ensure public users
  // only have access to explicitly allowed tools
  const channelRole = getChannelRole(options?.messageProvider ?? "unknown");
  const roleFilteredTools = toolAccessFilter.filterToolsForRole(
    channelRole,
    withAbort
  );

  // Log filtering results for debugging
  if (channelRole === "public" && withAbort.length !== roleFilteredTools.length) {
    const filteredOut = withAbort.filter(
      (tool) => !roleFilteredTools.some((t) => t.name === tool.name)
    );
    logWarn(
      `tools: Filtered out ${filteredOut.length} tools for public channel '${options?.messageProvider}': ${filteredOut.map((t) => t.name).join(", ")}`
    );
  }
